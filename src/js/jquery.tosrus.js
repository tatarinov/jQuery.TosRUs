...
(function( $ ) {
	...
	*/
	...
	$[ _PLUGIN_ ].prototype = {
		...
		prev: function( slides )
		{
			if ( !$.isNumeric( slides ) )
			{
				slides = this.opts.slides.slide;
			}
			this.slideTo( this.slides.index - slides );
...
//			width	: null,				//	auto, max 100%
	...
	*/
	$[ _PLUGIN_ ].effects	= {
		'slide': function( left )
		{
			this.nodes.$sldr.css( 'transform', 'translateX(' + left + ')' );
		},
		'fade': function( left )
		{
			_f.transitionend( this.nodes.$sldr,
	...
	*/
	function initPlugin()
	{
		...
		//	Eventnames
		_e.add( 'open opening close closing prev next slideTo sliding click keyup scroll orientationchange load loading resize transitionend webkitTransitionEnd' );
